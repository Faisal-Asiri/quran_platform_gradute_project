...
if ($result->num_rows > 0) {echo "تقدمك الحالي  <img src=../../images/in-progress.png height=50px><table id='memoriestable'><tr>";
...
<div class="tabbable"> <!-- Only required for left/right tabs -->
  <ul class="nav nav-tabs">
    ...
    <li><a href="#tab5" data-toggle="tab">الحساب العائلي</a></li>
  </ul>
...
U.username='$user' AND U.username=L.username  GROUP BY L.id  
;";
	$result = mysqli_query($connection, $sql);
	if ($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
...
FROM  login L,family F WHERE
F.username='$user' AND F.username=L.username   
;";
	$result = mysqli_query($connection, $sql);
	if ($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
		 echo"<br>العضو : " .$row["mbr1"]."<br>";
		 echo"<br>العضو : " .$row["mbr2"]."<br>";
		 echo"<br>العضو : " .$row["mbr3"]."<br>";
		 echo"<br>العضو : " .$row["mbr4"]."<br>";
		 
		

		}echo "<a href=family.php>ادارة الحساب العائلي</a>";
	}else{ echo"لايوجد افراد مضافين ! هل ترغب في تحويل حسابك الى حساب عائلي ؟ ";

echo "<a href=family.php>فتح حساب عائلي</a>";
}
